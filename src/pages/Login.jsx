...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        console.log("Respons server:", JSON.stringify(data, null, 2));

        if (!data.user) {
            console.error("Error: Data user tidak ditemukan dalam respons server!");
            return;
        }

        console.log("User data:", data.user);
        console.log("User Role:", data.user?.role);
        console.log("is_narasumber:", data.user?.is_narasumber, "Tipe:", typeof data.user?.is_narasumber);
        console.log("group_id:", data.user?.group_id);
        ...
        if (data.user.group_id || null) {
            localStorage.setItem("group_id", (data.user.group_id || null));
        } else {
            localStorage.removeItem("group_id");
        }

        console.log("Cek Navigasi -> Role:", data.user.role, "is_narasumber:", data.user.is_narasumber, "Tipe:", typeof data.user.is_narasumber);

        if (["admin", "admin_group"].includes(data.user.role) || data.user.is_narasumber == 1) {
          console.log("Navigasi ke /Admin");
          navigate("/Admin");
      } else if (data.user.role === "user") {
          console.log("Navigasi ke /User");
          navigate("/User");
      } else {
          console.error("Role tidak dikenali:", data.user.role);
